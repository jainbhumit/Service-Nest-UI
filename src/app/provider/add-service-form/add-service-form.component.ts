...
import { AddService, ProviderDetail, ProviderServiceDetail } from '../../models/service.model';
...
export class AddServiceFormComponent implements OnInit{
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: { category: string , is_update:boolean ,service_detail:ProviderServiceDetail},
  ) { 
  ...
  onSubmit() {
    if (this.addServiceForm.valid) {
      ...
      if (this.isUpdate) {
        this.providerService.updateService(body,this.data.service_detail.id).subscribe({
          next: (response) => {
